...
namespace AIOS\Communities\Config;
...
class Config{
	...
	public static function template_location( $theme_path ) {
		$folder = [
			[
				'path' =>AIOS_COMMUNITIES_DIR . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . $theme_path . DIRECTORY_SEPARATOR, /** This resides in the plugin **/
				'url' => AIOS_COMMUNITIES_URL . 'templates/' . $theme_path . '/',
				'location_name' => 'core'
			],
			[
				'path' => realpath( get_stylesheet_directory() ) . DIRECTORY_SEPARATOR . 'aios-communities' . DIRECTORY_SEPARATOR, /** This resides in the current theme or child theme. Gets deleted when theme is deleted.  **/
				'url' => get_stylesheet_directory_uri() . '/aios-communities/',
				'location_name' => 'active-theme'
			],
			[
				'path' => realpath( WP_CONTENT_DIR ) . DIRECTORY_SEPARATOR . 'aios-communities' . DIRECTORY_SEPARATOR, /** This resides in the wp-content folder to prevent deleting when upgrading themes. Recommended location. **/
				'url' => WP_CONTENT_URL . '/aios-communities/' . $theme_path . '/',
	...
	public static function get_template_location($page_theme) {
		...
		$filteredTemplates = $filteredTemplates ? $filteredTemplates['communities'][$page_theme] : [];
						...
						/** BEGIN: Check if each DIR **/
							foreach ( $files as $file) {
								...
								if ( is_dir( $template_path ) && @file_exists( $template_functions ) && !in_array( $file, $templates) ) {
									...
									$template_screenshot 	= @file_exists( $template_screenshot_dir ) ? $template_screenshot : AIOS_COMMUNITIES_URL_ASSETS_IMG . '/screenshot.jpg';
