...
/**
 * Sample URL:
 * /wp-json/aios-communities/ - This will get all the communities
 * Accepted URL @param:
 * agent_id(int) - Post ID.
 * agent_communities_sortby(selected-only / unselected-only / selected-first / unselected-first / date-created / alphanumeric ) -  This will get/except communities under agent_id.
 ...
 * s - Search Query
 *
 * Sample URL to Get Agents communities
 * /wp-json/aios-communities/v1/s?agent_id=894&agent_communities_sortby=selected-only - This will get all the communities under Agent ID(894)
 * /wp-json/aios-communities/v1/s?agent_id=894&agent_communities_sortby=unselected-only - This will get all the communities except Agent ID(894)
 */

if ( !class_exists( 'aios_communities_rest_api' ) ) {

	class aios_communities_rest_api {
		...
		public function custom_rest_api() {
			register_rest_route( 
				...
				array(
					...
					'callback' 	=> array( $this, 'get_all_communities' )
				)
			);
		 ...
		 * @return array
		 */
		public function get_all_communities( $request ) {
			...
			while ( $query->have_posts() ) {
				...
				$position         					= get_post_meta( $post_id, 'aios_communities_position', true );
				$image          					= get_post_meta( $post_id, 'aios_communities_image', true );
	...
	$aios_communities_rest_api = new aios_communities_rest_api();
